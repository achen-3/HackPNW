...
def draw_circle_button(text, x, y, radius, color, hover_color, border_color, border_width, action=None):
    mouse = pygame.mouse.get_pos()
    click = pygame.mouse.get_pressed()
    # Draw the border (a larger circle)
...
def draw_return_button():
	...
	draw_circle_button('Return', button_x, button_y, button_radius, (128,128,128), (105,105,105), (0, 0, 0), 5, return_to_start)
...
def draw_level_buttons():
	...
	for i, grade in enumerate(GRADES):
		...
		draw_circle_button(str(grade), x, HEIGHT / 2 - button_radius, button_radius, color, hover_color, (0, 0, 0), 5, make_grade_lambda(grade))
...
def draw_back_button():
	...
	draw_circle_button('Back', button_x, button_y, button_radius, (128,128,128), (105,105,105), (0, 0, 0), 5, back)
def reset_state():
	state['subject'] = None
	state['grade'] = None
	state['screen'] = 'title'
...
while True:
	for event in pygame.event.get():
		if event.type == pygame.QUIT:
			...
			if event.button == 1:
				if state['subject'] == 'Numerical Odyssey' and next_question_button_rect.collidepoint(event.pos) and user_input != '':
					user_input = ''
					CanInputMathAnswer = True
